...
class DisburseStatusController extends GetxController {
  final String reference;
  
  DisburseStatusController({required this.reference});
  ...
  void onInit() {
    ...
    if (reference.isNotEmpty) {
      // If we don't have an initial status yet, it's pending. 
      // If the UI already updated the status to successful/failed, startPolling will handle it.
      startPolling();
    } else {
  ...
  Future<void> performPoll() async {
    ...
    try {
      ...
      final res = await APPHttpHelper.get(
        "${APIConstants.merchantDisburseStatus}$reference",
        token,
      );
