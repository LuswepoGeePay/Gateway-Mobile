...
class CollectController extends GetxController {
  static CollectController get instance => Get.find();
  ...
  Future<Map<String, dynamic>?> submitCollection() async {
    ...
    try {
      ...
      final res = await APPHttpHelper.post(
        APIConstants.merchantCollect,
        token,
        data,
      );

      final resData = res['data'] ?? res;
      final hasRef = resData['transaction_reference'] != null || 
                     resData['transaction_ref'] != null || 
      ...
      final isGoodStatus = status == 'success' || status == 'successful' || 
                           status == 'pending' || status == 'processing' || status == 'created';

      if (hasRef || isGoodStatus) {
        return res;
      } else {
        String errorMsg = "Could not initiate collection.";
        if (res['message'] != null) {
